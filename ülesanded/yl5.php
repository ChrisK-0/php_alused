<?php
// Tüdrukud I
echo "<h1>Tüdrukud I</h1>";
$girls = array(
    'Mari', 'Kati', 'Juhanna', 'Miku', 'Liis',
    'Lisanna', 'Liisbeth', 'Merilin', 'Merlin', 'Piret',
    'Jane', 'Loona', 'Kelly', 'Viktoria', 'Agnes'
);
sort($girls);
foreach($girls as $nimi) {
    echo "$nimi <br>";
}
// Tüdrukud I
echo "<br><br><br><br>";
//Tüdrukud II
echo "<h1>Tüdrukud II</h1>";
echo "Esimesed kolm nime on: ".$girls[0].", ".$girls[1].", ".$girls[2]."<br><br>";
echo "Viimane nimi: ".end($girls)."<br><br>";
shuffle($girls);echo "Üks suvaline nimi: ".$girls[0];
//Tüdrukud II
echo "<br><br><br><br>";
...
echo "Autosid on kokku: ".count($cars).'<br>';
echo "VIN koode kokku: ".count($vincode).'<br>';
// samade autode lugemine
$same_car = array_count_values($cars);
echo "Toyotasid on kokku: ".$same_car['Toyota'].'<br>'."Audisid on kokku: ".$same_car["Audi"].'<br><br>';
echo "VIN koodid (< 17): ";
// vinkoodide näitamine, mis on lühemad kui 17 karakterit
foreach ($vincode as $vinsingle) {
  if (strlen($vinsingle) < 17) {
      echo $vinsingle." , ";
  }
}
//Autod
...
$palk_avg = array_sum($palgad)/count($palgad);
echo "Keskmine palk on: ".$palk_avg;
//Keskmised palgad
echo "<br><br><br><br>";
// Firmad
echo "<h1>Firmad</h1>";
...
foreach($firmad as $firma) {
    ...
    // "Kasutajal võimalik nime järgi eemaldada" poolik
    // echo '<input style=""> <button name="show_firma">';
// Firmad
echo "<br><br><br><br>";
...
// declaring a function called sorty, that magically sorts arrays
// from the longest string length to the lowest
function sorty($a, $b){
...
sort($hiina_nimed);
echo "Esimene on ".$hiina_nimed[0]." ja ".end($hiina_nimed)." on viimane.";
// Hiina nimed
echo "<br><br><br><br>";
...
echo 'Kasutaja:  <input type="text" name="username"><br>';
$username = $_GET['username'];
$user_search = array_search($username, $users, true);
if ($user_search = true){
    echo '<script language="javascript">';
    echo 'alert("See nimi on olemas!")';
    echo '</script>';
} else {
    echo '<script language="javascript">';
    echo 'alert("Seda nime veel pole")';
    echo '</script>';
}
echo '<input type="button" onclick="alerto()" value="Tšeki">';
